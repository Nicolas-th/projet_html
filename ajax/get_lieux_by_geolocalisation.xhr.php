	...
	if(isset($_POST['points'])){
		...
		foreach($tab_lieux as $lieu){
			...
			if(is_array($infos_lieu) && !empty($infos_lieu[0]) && !empty($infos_lieu[1])){
				...
				$sql = new SQL($dbh);
				// Récupération des lieux se trouvant autour du point
				$sql->prepare('SELECT *,( 6371 * acos(cos(radians(:lat)) * cos(radians(latitude)) * cos(radians(longitude) - radians(:lng)) + sin(radians(:lat)) * sin(radians(latitude)))) AS distance FROM places HAVING distance < 1 ORDER BY distance LIMIT 0 , 20');
